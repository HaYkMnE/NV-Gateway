...
export function Dashboard() {
  ...
  // ONE named cancel for the add-key form: the Cancel button and the Escape
  // keydown both call it, so the two paths can never drift apart.
  const cancelAddKey = () => { setAdding(false); setNewKey(''); setKeyError(''); setShowToken(false); };
  const add = async () => {
  ...
  return <div className="flex flex-col h-full p-4 sm:p-8 min-w-0">
    ...
    {adding && <div className="bg-surface border border-nvidia p-4 mb-4 flex flex-wrap gap-3" onKeyDown={(event) => { if (event.key === 'Escape' && !addKeyMutation.isPending) cancelAddKey(); }}><div className="flex-1 min-w-[220px]"><label htmlFor="new-key" className="sr-only">{t('key_label')}</label><div className="relative"><input id="new-key" dir="ltr" type={showToken ? 'text' : 'password'} value={newKey} onChange={(event) => setNewKey(event.target.value)} aria-invalid={Boolean(keyError)} aria-describedby={keyError ? "key-error" : undefined} className="w-full bg-bg border border-border p-2 pe-10 font-mono" autoFocus disabled={unavailable} /><button type="button" onClick={() => setShowToken((v) => !v)} onMouseDown={(event) => event.preventDefault()} aria-label={showToken ? t('hide_key') : t('show_key')} aria-pressed={showToken} className="absolute end-2 top-1/2 -translate-y-1/2 text-textMuted hover:text-accent-neon">{showToken ? <EyeOff aria-hidden size={16} /> : <Eye aria-hidden size={16} />}</button></div>{keyError && <p id="key-error" role="alert" className="text-error text-sm mt-2 break-words">{keyError}</p>}</div><button onClick={() => void add()} disabled={unavailable || addKeyMutation.isPending} className="bg-nvidia text-bg px-4 py-2 disabled:opacity-50">{t('save')}</button><button onClick={cancelAddKey} disabled={unavailable || addKeyMutation.isPending} className="px-3 text-textMuted">{t('cancel')}</button></div>}
    ...
    {state === 'stale' && <div role="status" className="border border-error/60 p-3 mb-3 text-textMuted">{t('stale')} <button onClick={() => void query.refetch()} className="text-nvidia ms-2">{t('retry')}</button></div>}
