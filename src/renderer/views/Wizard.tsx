...
const candidates = [8000, 12000, 24000];
  ...
  return <main className="h-full overflow-y-auto bg-bg px-4 py-4 sm:px-8 sm:py-8 flex items-start justify-center">
    <section className="bg-surface border border-border w-full min-w-0 max-w-[440px] max-h-[calc(100vh-2rem)] overflow-y-auto p-5 sm:p-8 shadow-2xl relative break-words" aria-labelledby="wizard-title">
      ...
      {step === 1 ? <fieldset className="grid gap-2"><legend className="sr-only">{t('language')}</legend>{languageOptions.map(({ value, labelKey }) => <label key={value} className={`p-3 border cursor-pointer flex items-center transition-colors ${lang === value ? 'border-nvidia text-nvidia bg-nvidia/10' : 'border-border hover:border-textMuted'}`}><input type="radio" name="language" value={value} checked={lang === value} onChange={() => void chooseLanguage(value)} className="me-3" /><span>{t(labelKey)}</span></label>)}</fieldset> : <div className="grid gap-4">
        ...
        <div aria-live="polite" className="text-sm text-textMuted">{scanning ? t('scanning') : autoPort ? <>{t('recommended')}: <bdi dir="ltr">{autoPort}–{autoPort + 1}</bdi></> : t('no_free_pair')}</div>
        {autoPort && <label className={`p-4 border flex items-center gap-3 cursor-pointer ${port === autoPort ? 'border-nvidia text-nvidia' : 'border-border'}`}><input type="radio" name="port" checked={port === autoPort} onChange={() => setPortInput(String(autoPort))} /><Zap aria-hidden size={16} /><bdi dir="ltr">{autoPort}–{autoPort + 1}</bdi>{port === autoPort && <Check aria-hidden size={16} />}</label>}
        <fieldset className="grid gap-2"><legend className="sr-only">{t('port')}</legend>{candidates.map((p) => { const state = statuses[p] ?? 'unknown'; return <label key={p} className={`p-3 border flex justify-between cursor-pointer ${port === p ? 'border-nvidia text-nvidia' : 'border-border'} ${state === 'in-use' || state === 'error' ? 'cursor-not-allowed text-textMuted' : ''}`}><span><input type="radio" name="port" checked={port === p} disabled={state !== 'available'} onChange={() => setPortInput(String(p))} className="me-3" /><bdi dir="ltr">{p}</bdi></span><span>{t(state.replace('-', '_'))}</span></label>; })}</fieldset>
        <div><label htmlFor="custom-port" className="block text-sm text-textMuted mb-2">{t('custom_port')}</label><input id="custom-port" dir="ltr" inputMode="numeric" value={portInput} onChange={(event) => setPortInput(event.target.value)} aria-invalid={Boolean(validation)} aria-describedby={errorKey ? "port-help port-error" : "port-help"} className="w-full bg-bg border border-border p-3 font-mono" /><p id="port-help" className="text-xs text-textMuted mt-2">{t('port_help')}</p>{errorKey && <p id="port-error" role="alert" className="text-sm text-error mt-1">{t(errorKey)}</p>}</div>
      </div>}
      <div className="mt-7 flex justify-end"><button type="button" onClick={() => void submit()} disabled={saving || (step === 2 && Boolean(validation))} className="bg-nvidia text-bg px-6 py-2 font-bold disabled:opacity-50">{saving ? <Loader2 aria-label={t('loading')} className="animate-spin" size={18} /> : t(step === 1 ? 'next' : 'save')}</button></div>
    </section>
  </main>;
}
