            ...
            if (isSuccessfulStatus(upstreamRes.statusCode)) {
                ...
                try {
                    ...
                    if (parsed && Array.isArray(parsed.data)) {
                        ...
                        res.writeHead(upstreamRes.statusCode, capResponseHeaders({ ...upstreamRes.headers, "content-length": Buffer.byteLength(body) }));
                        res.end(body);
                        settle({ success: true });
                        // Fire-and-forget background capability sweep for any
                        // model whose cached capability probe is stale or missing.
                        scheduleCapabilityRefresh(enrichedModelIds, {
                            keyProvider: () => getNextKey()?.key,
...
const server = http.createServer(async (req, res) => {
    ...
    let loggedKeyId = undefined;
    let loggedModel = undefined;

    const requestUrl = new URL(req.url, "http://127.0.0.1");
    req.on("close", () => {
        const duration = Date.now() - startedAt;
        const normalizedStatus = typeof res.statusCode === "number" && Number.isInteger(res.statusCode) ? res.statusCode : undefined;
    ...
    const allowedOrigin = isAllowedOrigin(origin, CORS_ALLOWLIST);
